...
class UserClass:
    ...
    def update_profile(self):
        ...
        if element == "(1) first name":
            ...
            self.cursor.execute(f"UPDATE users SET firstname = '{new_firstname}' WHERE username = '{self.username}';")
            ...
            self.cursor.execute(f"UPDATE users SET lastname = '{new_lastname}' WHERE username = '{self.username}';")
            self.cursor.fetchall()
            self.connection.commit()
            ...
            self.cursor.execute(f"UPDATE users SET password = '{new_password}' WHERE username = '{self.username}';")
            self.cursor.fetchall()
            self.connection.commit()
    ...
    def store_habit_in_db(self, new_habit):
        ...
        """
        self.cursor.execute("INSERT INTO habits VALUES(?, ?, ?, ?, ?)",
                         (new_habit.habit_name, new_habit.user_name, new_habit.category, new_habit.periodicity,
    ...
    def get_habit(self, habit_name):
        ...
        """
        self.cursor.execute(f"SELECT * FROM habits WHERE habit_name = '{habit_name}' AND owner = '{self.username}';")
        list_of_habits = self.cursor.fetchall()
        if len(list_of_habits) > 0:
            habit_name, owner, category, periodicity, datetime_of_creation = list_of_habits[0]
            habit = Habits.Habit(habit_name, owner, category, periodicity, datetime_of_creation)
            return habit
        else:
            return None
    ...
    def choose_predefined_habit(self):
        ...
        """
        self.cursor.execute(f"SELECT * FROM habits WHERE owner = '{self.username}';")
        list_of_habits = self.cursor.fetchall()
        ...
        if len(list_of_habits) > 0:
            ...
            if first_habit:
                ...
                new_habit = Habits.Habit(habit_name, owner, category, periodicity, datetime_of_creation)
    ...
    def delete_habit(self):
        ...
        existing_habit = self.get_habit(habit_name)
        if existing_habit:
            self.cursor.execute(f"DELETE FROM habits WHERE habit_name = '{habit_name}' AND owner = '{self.username}';")
            self.cursor.fetchall()
            self.connection.commit()
    ...
    def update_habit(self):
        ...
        if existing_habit:
            ...
            if element == "(1) category":
                ...
                self.cursor.execute(f"UPDATE habits SET category = '{new_category}' WHERE habit_name = '{to_change}' "
    ...
    def show_all(self):
        ...
        """
        self.cursor.execute(f"SELECT habit_name FROM habits WHERE periodicity IS NOT NULL AND owner = '{self.username}';")
    ...
    def show_weekly_habits(self):
        ...
        """
        self.cursor.execute(f"SELECT habit_name FROM habits WHERE periodicity = 'Weekly' AND owner = '{self.username}';")
        items = self.cursor.fetchall()
        habits = []
        for item in items:
            habits.append(item[0])
    ...
    def show_daily_habits(self):
        ...
        """
        self.cursor.execute(f"SELECT habit_name FROM habits WHERE periodicity = 'Daily' AND owner = '{self.username}';")
        items = self.cursor.fetchall()
        habits = []
        for item in items:
            habits.append(item[0])
    ...
    def is_completed(self):
        ...
        if existing_habit:
            ...
            self.cursor.execute("INSERT INTO progress VALUES(?, ?, ?, ?)",
    ...
    def get_habit_progress(self, habit_name, periodicity):
        ...
        """
        self.cursor.execute(f"SELECT datetime_of_completion FROM progress "
                         f"WHERE owner = '{self.username}' AND habit_name = '{habit_name}' "
                         f"AND periodicity = '{periodicity}';")
        user_progress = self.cursor.fetchall()
    ...
    def current_streak_overview(self):
        ...
        """
        self.cursor.execute(f"SELECT habit_name FROM habits WHERE owner = '{self.username}' AND periodicity = 'Daily';")
        items = self.cursor.fetchall()
        for item in items:
            habit_name = item[0]
        ...
        self.cursor.execute(f"SELECT habit_name FROM habits WHERE owner = '{self.username}' AND periodicity = 'Weekly';")
        items = self.cursor.fetchall()
        for item in items:
            habit_name = item[0]
    ...
    def longest_streak_overview(self):
        ...
        # daily habits
        self.cursor.execute(f"SELECT habit_name FROM habits WHERE owner = '{self.username}' AND periodicity = 'Daily';")
        all_daily_habits = self.cursor.fetchall()
        daily_habits = []
        ...
        # weekly habits
        self.cursor.execute(f"SELECT habit_name FROM habits WHERE owner = '{self.username}' AND periodicity = 'Weekly';")
        all_weekly_habits = self.cursor.fetchall()
        weekly_habits = []
